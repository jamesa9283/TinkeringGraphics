...
def normal_to_colour_blind(_normal_colour: (int, int, int), conversion_list: list) -> (int, int, int):
    ...
    blind_index = int(round(normal_hsv[0] * 150))
...
mode = 0 # Deuteranomaly Protanomaly Deuteranopia Tritanopia Tritanomaly

values = []
with open("blindnessValues.txt", 'r') as file:
    for i in range(mode + 1):
        valuesstr = file.readline()
